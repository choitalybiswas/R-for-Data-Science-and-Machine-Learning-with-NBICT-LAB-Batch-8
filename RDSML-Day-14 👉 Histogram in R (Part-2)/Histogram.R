...
myPlot = ggplot(dataset, aes(x = weight, fill = sex)) +
  ...
  geom_vline(data = group_means, aes(xintercept = grp.mean, colour = sex),
             linetype = "dashed") +
  theme(legend.position = "top")

myPlot

# Using custom color palletes
myPlot + scale_color_manual(values = c("#999999", "#E69F00")) +
  scale_fill_manual(values = c("#999999", "#E69F00"))

#or we can use for custom color palletes)
myPlot + scale_color_manual(values = c("red", "green")) +
  scale_fill_manual(values = c("orange", "blue"))


# Using color brewer color pallets
